...
- `HomeView`: 검색창 + 하단 탭바 + 결과 목록 화면입니다.
...
Kakao REST API 키는 코드에 직접 두지 않고 `Info.plist` 값을 통해 읽도록 구성했습니다.

### 7. 초기 진입 UX 보완

첫 화면 진입 시 짧은 준비 오버레이를 보여주고, 준비가 끝나면 검색창에 자동 포커스가 가도록 구성했습니다. 첫 입력 시점을 더 자연스럽게 만들기 위한 보완입니다.

### 8. 검색 결과 디테일 강화

검색 결과 카드에는 출처, 해상도, 날짜, 원문 링크를 함께 표시해 단순 목록을 넘어서 정보 확인이 가능하도록 구성했습니다.
...
5. 하단 탭바에서 북마크 탭으로 전환해 저장 목록 확인
6. 검색창이 비어 있을 때 최근 검색어 표시
...
- 초기 화면 준비 중에는 터치를 막고 준비 완료 후 검색창에 자동 포커스 적용
